#!/usr/bin/env python
""" sbox_analysis.py

    AES and SMS4 Sbox Analysis - a simple analysis of Sboxs that determines
    the number and size of the permutation subgroups in the transformation.

    Copyright (c) 2002 by Paul A. Lambert
    Read LICENSE.txt for license information.

    2008-06-18 fixed a few bugs, updated and added sms4
    2002-06-01 Original
"""
# The AES Sbox
aes_sbbytes = ()

# The SMS4 Sbox
sms4_sbbytes = ( )

def map_orbits_of_permutation(permutation):
    """ Creates a dictionary of permutations indexed by the first element of the cycle/orbit """
    gdict={}     # a dictionary of the orbits indexed by the first orbit element
    touched=[0 for i in range(len(permutation))]
    for i in range(len(permutation)):
        touched.append(0) 
    for i in range(len(permutation)):
        element = i
        cycle = []
        if not touched[element]:
            for j in range (i,len(permutation)):
                touched[element] = 1
                cycle.append(element)
                element = permutation[element]
                if element == i:
                    break
            gdict[cycle[0]]=cycle
        else:
            pass
    return gdict

def orbit_start_and_length(permutation):
    """ Returns a list of tuples (orbit start, orbit size) """
    v=[]
    z=map_orbits_of_permutation(permutation)
    for i in z.keys():
        v.append( [i, len(z[i])] )
    return v

def main():
    orbit_tuples = orbit_start_and_length(aes_sbbytes)
    print
    print 'The AES sbox contains ',
    print len(orbit_tuples),
    print 'permutation subgroups'
    print 'The AES sbox subgroups (start, length) are:'
    print orbit_tuples
    print
    orbit_tuples = orbit_start_and_length(sms4_sbbytes)
    print 'The SMS4 sbox contains ',
    print len(orbit_tuples),
    print 'permutation subgroups'
    print 'The SMS4 sbox subgroups (start, length) are:'
    print orbit_tuples
    #print '--- SMS4 cycles ---'
    #d = map_orbits_of_permutation(sms4_sbbytes)
    #for i in d:
    #    print d[i]

# Make this module runnable from the command prompt
if __name__ == "__main__":
    main()



